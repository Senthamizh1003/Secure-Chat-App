...
// };
// function newMessage() {
//     var message = $(".message-input input[type='text']").val();
...
//     // Prepare the data
//     var data = { from: getMe().id, to: currentChannelName, avatar: getMe().avatar };

//     if (message) {
//         var chatSymmetricKey = getChannels()[currentChannelName].channelKey;
//         data.msg = message.symEncrypt(chatSymmetricKey);
//     }
...
// }
function newMessage() {
    const messageInput = $(".message-input input[type='text']");
    ...
    files.forEach((file) => {
        ...
        reader.onload = (e) => {
            ...
            if (data.attachments.length === files.length) {
                // Send data after all attachments are ready
                socket.emit("msg", data);
                fileInput.value = ""; // Reset file input
                $("#attachmentList").empty(); // Clear attachment list
            }
        };
        reader.readAsDataURL(file);
    });
...
//     } else {
//         data.state = "replies";
//         data.name = lstUsers[data.from]?.username || "Unknown User";
//     }

//     var msg = "";
//     if (data.msg) {
//         var symmetricKey = getChannels()[currentChannelName].channelKey;
//         msg = data.msg.symDecrypt(symmetricKey);
//     }

//     var fileContent = "";
//     if (data.file) {
//         fileContent = `<a href="${data.file.data}" download="${data.file.name}" class="attachment">
//                            <i class="fa fa-file"></i> ${data.file.name}
//                        </a>`;
//     }
...
//     const symmetricKey = getChannels()[currentChannelName]?.channelKey;
//     const decryptedMessage = data.msg ? data.msg.symDecrypt(symmetricKey) : "";
...
//             .map(
//                 (file) =>
//                     `<a href="${file.data}" download="${file.name}" class="attachment">
...
//         `<li class="${data.state}">
//             <img src="${data.avatar}" title="${data.name}" />
//             <p>${attachmentsHTML}${decryptedMessage}</p>
//         </li>`
//     );
...
//             <img src="${data.avatar}" title="${data.name}" />
//             <p>
//                 ${attachmentsHTML}${decryptedMessage}
...
function appendMessage(data) {
    ...
    // Determine tick icon based on message status
    let tickHTML = "";
    if (data.status === "sent") {
        tickHTML = `<i class="tick grey-tick fa fa-check"></i>`;
    } else if (data.status === "delivered") {
        tickHTML = `<i class="tick grey-tick fa fa-check"></i><i class="tick grey-tick fa fa-check"></i>`;
    } else if (data.status === "read") {
        tickHTML = `<i class="tick blue-tick fa fa-check"></i><i class="tick blue-tick fa fa-check"></i>`;
    }
    ...
    Array.from(fileInput.files).forEach((file, index) => {
        attachmentList.append(
            `<div class="attachment-item">
                <span>${file.name}</span>
                <i class="fa fa-times remove-attachment" data-index="${index}"></i>
            </div>`
        );
    });
...
(function ($) {
	...
	$("#logoutButton").on("click", function () {
		...
		location.reload();    // Refresh page to force login
	});
