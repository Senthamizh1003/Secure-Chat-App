    ...
    io.on('connection', function (socket) {
        ...
        socket.on('login', data => {
            ...
            if (user) { // exist user                
                ...
                userSigned(user, socket);
                // Notify user that this will be their password from now on
            socket.emit("alert", `This will be your password from now on. Please remember it!`);
            }
        }); // login
...
function defineSocketEvents(socket) {
    ...
    //         var msg = manager.messages[channel.name] || (manager.messages[channel.name] = []);
    ...
    // });
    socket.on("msg", (data) => {
        const from = socket.user || manager.findUser(socket.id);
        const channel = manager.channels[data.to];
    
        if (from && channel && channel.users.includes(from.id)) {
            const msg = manager.messages[channel.name] || (manager.messages[channel.name] = []);
    
            data.date = Date.now();
            data.type = "msg";
    
            if (data.attachments && data.attachments.length > 0) {
                console.log(`Received ${data.attachments.length} attachment(s):`);
                data.attachments.forEach((file) => console.log(`- ${file.name}`));
            }
